...
function magreload()
	while activeItem.ownerHasItem(self.gunConfig.compatibleAmmo[1]) and self.gunConfig.magazineCurrent < self.gunConfig.magazineMax do
		activeItem.takeOwnerItem(self.gunConfig.compatibleAmmo[1])
...
function shoot()
	if self.gunConfig.compatibleAmmo then
		...
		if self.gunConfig.magazineMax then
			...
			itemammo = activeItem.takeOwnerItem(self.gunConfig.compatibleAmmo[1])
		end
